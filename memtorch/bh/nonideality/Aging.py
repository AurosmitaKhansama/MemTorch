...
def apply_aging(layer, lrs_proportion, hrs_proportion, electroform_proportion, r_off, r_on):
    ...
    def apply_aging_to_crossbar(crossbar, lrs_proportion, hrs_proportion, r_max, r_min):
        crossbar_min = (
        ...
        for index in crossbar_min_indices:
            crossbar.conductance_matrix[index] = 1/r_max
        ...
        for index in crossbar_max_indices:
            crossbar.conductance_matrix[index] = 1/r_min
    ...
    for i in range(len(layer.crossbars)):
        layer.crossbars[i] = apply_aging_to_crossbar(
            layer.crossbars[i], lrs_proportion, hrs_proportion, r_off, r_on
        )
